...
import org.ideademo.climatologies.services.util.PDFStreamResponse;
...
*/  
  ...
  public List<Climatology> getList()
  {
    ...
    List <Climatology> alst = session.createCriteria(Climatology.class).list();
    total = alst.size();

	
    // then makes lists and sublists as per the search criteria 
    List<Climatology> xlst=null; // xlst = Query by Example search List
    if(example != null)
    {
       Example ex = Example.create(example).excludeFalse().ignoreCase().enableLike(MatchMode.ANYWHERE);
       
       xlst = session.createCriteria(Climatology.class).add(ex).list();
       
       
       if (xlst != null)
       {
    	   logger.info("Climatology Example Search Result List Size  = " + xlst.size() );
    	   Collections.sort(xlst);
       }
       else
       {
         logger.info("Climatology Example Search result did not find any results...");
       }
    }
    
    List<Climatology> tlst=null;
    if (searchText != null && searchText.trim().length() > 0)
    {
      ...
      try
      {
        ...
        while (stream.incrementToken()) 
         {
    	   ...
    	   logger.info("Adding search token " +  token + " to look in Climatologys database");
       ...
       {
    	   logger.warn("Climatologys Text Search: Encountered problem tokenizing search term " + searchText);
    	   logger.warn(ioe);
       }
       ...
       tlst = fullTextSession.createFullTextQuery(luceneQuery, Climatology.class).list();
       if (tlst != null) 
       {
    	   logger.info("TEXT Search for " + searchText + " found " + tlst.size() + " Climatologys records in database");
    	   Collections.sort(tlst);
       }
       else
       {
          logger.info("TEXT Search for " + searchText + " found nothing in Climatologys");
       }
    }
    ...
    if (example == null && (searchText == null || searchText.trim().length() == 0))
    {
    	...
    	{
    	  logger.info ("Returing all " + alst.size() + " Climatologys records");
          Collections.sort(alst);
    	}
    	else
    	{
    	  logger.warn("No Climatology records found in the database");
    	}
    	retrieved = total;
        return alst; 
    }
    else if (xlst == null && tlst != null)
    {
    	...
    	logger.info("Returing " + tlst.size() + " Climatologys records as a result of PURE text search (no QBE) for " + searchText);
    	retrieved = tlst.size();
    	return tlst;
    }
    else if (xlst != null && tlst == null)
    {
    	...
    	logger.info("Returning " + xlst.size() + " Climatologys records as a result of PURE Query-By-Example (QBE), no text string");
    	retrieved = xlst.size();
    	return xlst;
    }
    else 
    {
    	...
    	{
          logger.info("Returing " + tlst.size() + " Climatologys records as a result of ONLY text search, QBE pulled up ZERO records for " + searchText);
          retrieved = tlst.size();
          return tlst;
    	}
    	...
    	{
          logger.info("Returning " + xlst.size() + " Climatologys records as a result of ONLY Query-By-Example (QBE), text search pulled up NOTHING for string " + searchText);
          retrieved = xlst.size();
          return xlst;
    	}
    	
    	
    	List <Climatology> ivec = new Vector<Climatology>();
    	// if both are empty, return this Empty vector. 
    	if (xlst.size() == 0 && tlst.size() == 0)
    	{
          logger.info("Neither QBE nor text search for string " + searchText +  " pulled up ANY Climatologys Records.");
          retrieved = 0;
          return ivec;
    	}
    	...
    	Iterator<Climatology> xiterator = xlst.iterator();
    	while (xiterator.hasNext()) 
    	{
          Climatology x = xiterator.next();
          Long xid = x.getId();
    		
          Iterator<Climatology> titerator = tlst.iterator();
    		while(titerator.hasNext())
    		{
        		Climatology t = titerator.next();
        		Long tid = t.getId();
    	...
    	logger.info("Returning " + ivec.size() + " Climatologys records from COMBINED (text, QBE) Search");
    	retrieved = ivec.size();
    	return ivec;
    }
 ...
 */
  ...
  public StreamResponse onSelectedFromPdf() 
  {
      ...
      // Return response
      return new PDFStreamResponse(is,"PaCISClimatologies" + System.currentTimeMillis());
  }
  ...
  private InputStream getPdfTable(List list) 
  {
      ...
      try {
              ...
              document.add(new Paragraph("PaCIS Climatologies " + formatter.format(date)));
              ...
              // create table, 2 columns
           	Iterator<Climatology> iterator = list.iterator();
           	int count=0;
       		while(iterator.hasNext())
      		{
       			count++;
                Climatology climatology = iterator.next();
                // create table, 2 columns
                String acronym = StringUtils.trimToEmpty(climatology.getCode());
                String name = StringUtils.trimToEmpty(climatology.getName());
                String description = StringUtils.trimToEmpty(climatology.getDescription());
                String leadAgencies = StringUtils.trimToEmpty(climatology.getOrganization());
                String contacts = StringUtils.trimToEmpty(climatology.getContact());
                String url = StringUtils.trimToEmpty(climatology.getUrl());
  ...
  {
      Climatology climatology =  (Climatology) session.load(Climatology.class, id);
      // step 1: creation of a document-object
      Document document = new Document();
      ...
      document.close();
      ByteArrayInputStream bais = new ByteArrayInputStream(baos.toByteArray());
      return new PDFStreamResponse(bais,"PaCISClimatology" + System.currentTimeMillis());
  }	  
  
  ///////////////////////////////////////////////////////
  // private methods 
  
  private void clearRegions(Climatology climatology)
  {
   	climatology.setCentralNorthPacific(false);
  	climatology.setWesternNorthPacific(false);
  	climatology.setSouthPacific(false);
  	climatology.setPacificBasin(false);
  	climatology.setGlobal(false);
  }
  
  private void clearEcv(Climatology climatology)
  {
	climatology.setAtmosphericData(false);
	climatology.setOceanicData(false);
	climatology.setTerrestrialData(false);
  }
  
  private void clearPhenomena(Climatology climatology)
  {
	climatology.setDrought(false);
	climatology.setRainfall(false);
	climatology.setFlooding(false);
	climatology.setBleaching(false);
	climatology.setOtherPhenomena(false);
  }
  
  private void clearSector(Climatology climatology)
  {
	climatology.setHealth(false);
	climatology.setFreshWater(false);
	climatology.setEnergy(false);
	climatology.setTransportation(false);
	climatology.setPlanning(false);
	climatology.setSocioCultural(false);
	climatology.setAgriculture(false);
	climatology.setRecreation(false);
	climatology.setEcological(false);
	climatology.setOtherSector(false);
  }

  private void clearMethodologies(Climatology climatology)
  {
	climatology.setInsitu(false);
	climatology.setRemote(false);
	climatology.setStatistical(false);
	climatology.setDynamical(false);
  }
  /*
  private void clearTimescales(Climatology climatology)
  {
	 climatology.setPast(false);
	 climatology.setCurrent(false);
	 climatology.setFuture(false);
	 climatology.setOneMonth(false);
	 climatology.setThreeMonths(false);
	 climatology.setSixMonths(false);
  }
  */
